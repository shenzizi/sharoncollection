var hardcode_viewingbubble_json_oneweek = {"viewing": {"新台灣加油": 0.6, "正晶限時批": 0.36, "新聞面對面": 0.49, "新聞龍捲風": 0.58, "少康戰情室": 0.4, "新聞深喉嚨": 0.55}}, 
    hardcode_viewingbubble_jsonvalue_oneweek=[0.6,0.36,0.49,0.58,0.4,0.55]
var hardcode_viewingbubble_json_onemonth = {"viewing": {"新台灣加油": 0.63, "正晶限時批": 0.6, "新聞面對面": 0.39, "新聞龍捲風": 0.4, "少康戰情室": 0.25, "新聞深喉嚨": 0.32}}, 
    hardcode_viewingbubble_jsonvalue_onemonth=[0.63,0.6,0.39,0.4,0.25,0.32]
var hardcode_viewingbubble_json_oneyear = {"viewing": {"新台灣加油": 0.62, "正晶限時批": 0.47, "新聞面對面": 0.32, "新聞龍捲風": 0.5, "少康戰情室": 0.29, "新聞深喉嚨": 0.37}}, 
    hardcode_viewingbubble_jsonvalue_oneyear=[0.62,0.47,0.32,0.5,0.29,0.37]

var hardcode_pie_barline_data_oneweek=[{name:"新台灣加油",response:100,adcost:25000,cpm:450},{name:"正晶限時批",response:20,adcost:35000,cpm:400},{name:"新聞面對面",response:50,adcost:20000,cpm:450},{name:"新聞龍捲風",response:6,adcost:15000,cpm:250},{name:"少康戰情室",response:10,adcost:15000,cpm:550},{name:"新聞深喉嚨",response:10,adcost:37000,cpm:370}]; 
var hardcode_pie_barline_data_onemonth=[{name:"新台灣加油",response:150,adcost:28000,cpm:475},{name:"正晶限時批",response:40,adcost:32000,cpm:380},{name:"新聞面對面",response:50,adcost:20000,cpm:420},{name:"新聞龍捲風",response:8,adcost:20000,cpm:280},{name:"少康戰情室",response:12,adcost:15000,cpm:650},{name:"新聞深喉嚨",response:20,adcost:33000,cpm:350}]; 
var hardcode_pie_barline_data_oneyear=[{name:"新台灣加油",response:180,adcost:26000,cpm:400},{name:"正晶限時批",response:75,adcost:31000,cpm:400},{name:"新聞面對面",response:55,adcost:22000,cpm:400},{name:"新聞龍捲風",response:16,adcost:22000,cpm:230},{name:"少康戰情室",response:14,adcost:25000,cpm:670},{name:"新聞深喉嚨",response:35,adcost:28000,cpm:320}]; 


var hardcode_bardata_oneweek=[{name:"新台灣加油",pnvalue:50},{name:"正晶限時批",pnvalue:20},{name:"新聞面對面",pnvalue:15},{name:"新聞龍捲風",pnvalue:5},{name:"少康戰情室",pnvalue:2},{name:"新聞深喉嚨",pnvalue:3},{name:"新台灣加油",pnvalue:-20},{name:"正晶限時批",pnvalue:-2},{name:"新聞面對面",pnvalue:-6},{name:"新聞龍捲風",pnvalue:-5},{name:"少康戰情室",pnvalue:-4},{name:"新聞深喉嚨",pnvalue:0}]; 
var hardcode_bardata_onemonth=[{name:"新台灣加油",pnvalue:80},{name:"正晶限時批",pnvalue:50},{name:"新聞面對面",pnvalue:25},{name:"新聞龍捲風",pnvalue:25},{name:"少康戰情室",pnvalue:6},{name:"新聞深喉嚨",pnvalue:4},{name:"新台灣加油",pnvalue:-40},{name:"正晶限時批",pnvalue:-10},{name:"新聞面對面",pnvalue:-6},{name:"新聞龍捲風",pnvalue:-10},{name:"少康戰情室",pnvalue:-8},{name:"新聞深喉嚨",pnvalue:-2}]; 
var hardcode_bardata_oneyear=[{name:"新台灣加油",pnvalue:120},{name:"正晶限時批",pnvalue:40},{name:"新聞面對面",pnvalue:35},{name:"新聞龍捲風",pnvalue:55},{name:"少康戰情室",pnvalue:9},{name:"新聞深喉嚨",pnvalue:9},{name:"新台灣加油",pnvalue:-48},{name:"正晶限時批",pnvalue:-42},{name:"新聞面對面",pnvalue:-12},{name:"新聞龍捲風",pnvalue:-25},{name:"少康戰情室",pnvalue:-14},{name:"新聞深喉嚨",pnvalue:-10}]; 

var hardcode_layout_data = [
    { "len": 4, "color": "#8dd3c7", "label": "新聞面對面", "id": "新聞面對面"},
    { "len": 4, "color": "#ffffb3", "label": "正晶限時批", "id": "正晶限時批" },
    { "len": 4, "color": "#bebada", "label": "新聞龍捲風", "id": "新聞龍捲風" },
    { "len": 4, "color": "#fb8072", "label": "新聞深喉嚨", "id": "新聞深喉嚨" },
    { "len": 4, "color": "#b3de69", "label": "少康戰情室", "id": "少康戰情室" },
    { "len": 4, "color": "#fdb462", "label": "新台灣加油", "id": "新台灣加油" },
];

//viewing
var hardcode_viewing = [
    ['新聞面對面',0,1,0.64],
    ['新聞面對面',1,2,0.64],
    ['新聞面對面',2,3,0.64],
    ['新聞面對面',3,4,0.49],
    ['正晶限時批',0,1,0.51],
    ['正晶限時批',1,2,0.52],
    ['正晶限時批',2,3,0.49],
    ['正晶限時批',3,4,0.33],
    ['新聞龍捲風',0,1,0.47],
    ['新聞龍捲風',1,2,0.49],
    ['新聞龍捲風',2,3,0.43],
    ['新聞龍捲風',3,4,0.55],
    ['新聞深喉嚨',0,1,0.46],
    ['新聞深喉嚨',1,2,0.48],
    ['新聞深喉嚨',2,3,0.47],
    ['新聞深喉嚨',3,4,0.6],
    ['少康戰情室',0,1,0.29],
    ['少康戰情室',1,2,0.27],
    ['少康戰情室',2,3,0.29],
    ['少康戰情室',3,4,0.45],
    ['新台灣加油',0,1,0.85],
    ['新台灣加油',1,2,0.79],
    ['新台灣加油',2,3,0.85],
    ['新台灣加油',3,4,0.57],
];
//c.p.m
var hardcode_cpm = [
    ['新聞面對面',0,1,194],
    ['新聞面對面',1,2,194],
    ['新聞面對面',2,3,194],
    ['新聞面對面',3,4,253],
    ['正晶限時批',0,1,195],
    ['正晶限時批',1,2,191],
    ['正晶限時批',2,3,203],
    ['正晶限時批',3,4,301],
    ['新聞龍捲風',0,1,370],
    ['新聞龍捲風',1,2,355],
    ['新聞龍捲風',2,3,404],
    ['新聞龍捲風',3,4,316],
    ['新聞深喉嚨',0,1,378],
    ['新聞深喉嚨',1,2,362],
    ['新聞深喉嚨',2,3,370],
    ['新聞深喉嚨',3,4,290],
    ['少康戰情室',0,1,428],
    ['少康戰情室',1,2,460],
    ['少康戰情室',2,3,428],
    ['少康戰情室',3,4,276],
    ['新台灣加油',0,1,146],
    ['新台灣加油',1,2,157],
    ['新台灣加油',2,3,146],
    ['新台灣加油',3,4,218],
];


function tooltip(){ 
  var tooltipvalue=[]; 
    for (i=0;i<hardcode_viewing.length;i++){ 
     var tooltipobj={};    
      tooltipobj.name=hardcode_viewing[i][0];
      tooltipobj.value=hardcode_viewing[i][3];
        if (i%4==0){
          tooltipobj.date="12/22"
        }else if(i%4==1){
          tooltipobj.date="12/23"
        }else if(i%4==2){
          tooltipobj.date="12/24"
        }else{
          tooltipobj.date="12/25"
        }
      tooltipvalue.push(tooltipobj);
    }    
    return tooltipvalue
}

function barlinechart(){
    var chartvalue_viewing=[];
    for (i=0;i<hardcode_viewing.length;i++){ 
        viewingvalue=hardcode_viewing[i][3];
        cpmvalue=hardcode_cpm[i][3];        
        name=hardcode_viewing[i][0];
        chartvalue_viewing.push(viewingvalue,cpmvalue,name);
    }
    return chartvalue_viewing
}